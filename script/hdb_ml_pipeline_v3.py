"""
HDB Resale Price Prediction Pipeline  —  v3
...
Changes vs v2 (PCA-informed cleanup from 00-PCA-analysis.ipynb):
  - Dropped redundant columns identified by PCA:
  ...
  - Added interaction: year_completed_x_floor_area
      (year_completed is top PC1 driver for Building group, corr=0.632 with price)
"""
...
def feature_engineering(df, mall_dist_median):
    ...
    # year_completed is the top Building PC1 driver (corr=0.632 with price)
...
# ─── Select Features ──────────────────────────────────────────────────────────
# Redundant columns removed (PCA analysis):
#   floor_area_sqft — unit conversion duplicate of floor_area_sqm
#   hdb_age         — near-perfect negative of year_completed
#   lower, upper, mid — mid_storey already encodes storey position
...
sub.to_csv('../submission/submission_ensemble_v3.csv', index=False)
print("\nSubmission saved to submission/submission_ensemble_v3.csv")
print(sub.head())
