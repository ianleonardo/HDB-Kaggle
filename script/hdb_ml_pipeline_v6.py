"""
HDB Resale Price Prediction Pipeline  —  v6
Primary model : CatBoost
Secondary     : LightGBM, XGBoost
Metric        : RMSE
Validation    : 5-fold KFold OOF stacking; ensemble weights optimised on the
                full out-of-fold predictions (no row ever influences its own weight).

Changes vs v5:
  - Reverted to v4 feature set (removed cbd_dist_km, building_age_at_tranc,
    town_year_price_psm — all were redundant / slightly harmful)
  - Single 80/20 split replaced with 5-fold KFold OOF:
      * Each model trained 5 times; each training row predicted exactly once
        by a model that never saw it (true OOF — no leakage in weight optimisation)
      * Test predictions = mean of 5 fold predictions (lower variance)
      * Ensemble weights optimised on full OOF array vs y_train
      * Per-fold RMSE printed to diagnose fold variance
"""
...
def print_metrics(label, y_true, y_pred):
    ...
    print(f"  {label:<36}  RMSE={rmse:>10,.0f}  MAE={mae:>10,.0f}  MAPE={mape:>6.2f}%  R²={r2:.4f}")
...
# Note: encoded on full train set (minor leakage inside folds, negligible given
# smoothing and dataset size of 150k rows).
...
sub.to_csv('../submission/submission_ensemble_v6.csv', index=False)
print("\nSubmission saved to submission/submission_ensemble_v6.csv")
print(sub.head())
