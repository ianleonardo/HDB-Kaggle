"""
HDB Resale Price Prediction Pipeline — v13
...
Outputs  : Two submissions
             submission_v13_8020.csv  — 80/20 random split
             submission_v13_5fold.csv — 5-fold OOF (recommended)

Key change vs v12 — KD-tree radius search replaces geohash encoding:

  v12 problem (Edge Problem):
    Geohash divides space into discrete rectangles.  Two transactions 10 m
    apart on opposite sides of a cell boundary get completely different
    encodings even though their location-based prices should be nearly
    identical.  geo6_nbr_price (9-cell pool) softened but did not remove
    this discontinuity.

  v13 fix — continuous spatial smoothing via cKDTree:
    For each transaction, average the prices of ALL training transactions
    within a fixed radius.  No cells → no boundaries → fully continuous
    price surface.  Stats are computed from training rows only → leakage-free.

  Three spatial features (all leakage-free):
    spatial_500m_te   : smoothed mean price within 500 m  (~block level)
    spatial_2000m_te  : smoothed mean price within 2000 m (~district level)
    spatial_500m_psm  : smoothed mean price-per-sqm within 500 m

  No external spatial library required — uses scipy.spatial.cKDTree.
"""
...
# Spatial radius search parameters
# Singapore: 1° lat ≈ 111 000 m,  1° lon ≈ 110 970 m  (cos 1.35° ≈ 0.9997)
LAT_M   = 111_000.0
LON_M   = 110_970.0
RADII_M = [500, 2000]   # metres — fine (block) and broad (district)
...
def _smooth(count, grp_mean, global_mean, k=10):
    """Sigmoid smoothing: pull small groups toward the global mean."""
    s = 1.0 / (1.0 + np.exp(-(count - k) / k))
    return global_mean * (1 - s) + grp_mean * s


def _latlon_to_m(df):
    """Convert Latitude/Longitude to approximate metres (2-D Euclidean)."""
...
def target_encode_fold(df_tr, df_val, df_te, cols, target_col, k=10):
    ...
    Statistics are derived exclusively from df_tr.
    """
...
def spatial_radius_encode_fold(df_tr, df_val, df_te, target_col,
    ...
    For each query point, this function finds all TRAINING transactions within
    `radius` metres and computes their smoothed mean price.  Because the KD-tree
    is built from training coordinates only, val/test points never influence
    their own encoding → zero leakage.

    Unlike geohash, there are no cell boundaries: two physically close
    transactions always receive similar encoded values regardless of where
    they fall relative to any grid.

    Features produced (one set per radius in `radii_m`):
        spatial_{r}m_te   — smoothed mean resale_price within r metres
    Plus one price-per-sqm feature at the finest radius:
        spatial_{r0}m_psm — smoothed mean price-per-sqm within r0 metres

    Parameters
    ----------
    df_tr, df_val, df_te : DataFrames with 'Latitude', 'Longitude' columns.
    target_col           : target column name (present only in df_tr).
    radii_m              : list of radii in metres (default: RADII_M).
    ...
    # Build KD-tree from training coordinates only
    coords_tr = _latlon_to_m(df_tr)
    tree      = cKDTree(coords_tr)

    def _encode_radius(query_coords, values_tr, global_val, radius):
        """Vectorised smoothed mean for all query points at a given radius."""
        # query_ball_point returns a list (one per point) of neighbour indices
        neighbours = tree.query_ball_point(query_coords, r=radius,
                                           workers=-1)   # parallel
        ...
        for i, nbr in enumerate(neighbours):
            if len(nbr) == 0:
                ...
                out[i] = _smooth(len(nbr), values_tr[nbr].mean(),
                                 global_val, k)
        return out
    ...
    for r in radii_m:
        ...
        tr_enc[feat]  = _encode_radius(coords_tr,  prices_tr, global_mean, r)
        val_enc[feat] = _encode_radius(coords_val, prices_tr, global_mean, r)
        te_enc[feat]  = _encode_radius(coords_te,  prices_tr, global_mean, r)

    # Price-per-sqm at the finest (most local) radius
    r0   = min(radii_m)
    feat = f'spatial_{r0}m_psm'
    tr_enc[feat]  = _encode_radius(coords_tr,  psm_tr, global_psm, r0)
    val_enc[feat] = _encode_radius(coords_val, psm_tr, global_psm, r0)
    te_enc[feat]  = _encode_radius(coords_te,  psm_tr, global_psm, r0)
...
# ══════════════════════════════════════════════════════════════════════════════
# FEATURE ENGINEERING  (no target/spatial encoding here)
# ══════════════════════════════════════════════════════════════════════════════
...
# ══════════════════════════════════════════════════════════════════════════════
# FEATURE SELECTION
# town / planning_area excluded here — added as _te columns per fold.
# Spatial features (spatial_*) added per fold via KD-tree encoding.
# ══════════════════════════════════════════════════════════════════════════════
...
# Spatial feature names derived from RADII_M
...
# Target encoding (town, planning_area) — from training rows only
...
# Spatial encoding — KD-tree built on training coordinates only
print("  Computing spatial encodings (KD-tree)...", end=' ', flush=True)
sp_tr_a, sp_val_a, sp_te_a = spatial_radius_encode_fold(
    df_tr=df_tr_a, df_val=df_val_a, df_te=test_fe, target_col=TARGET,
)
print("done")

# Merge all encoding dicts
tr_enc_a  = {**tr_enc_a,  **sp_tr_a}
val_enc_a = {**val_enc_a, **sp_val_a}
te_enc_a  = {**te_enc_a,  **sp_te_a}
...
for fold, (tr_idx, val_idx) in enumerate(kf.split(np.arange(len(train_fe))), 1):
    ...
    # Target encoding — each fold independent → zero leakage
    tr_enc, val_enc, te_enc = target_encode_fold(
        df_tr=df_tr_f, df_val=df_val_f, df_te=test_fe,
        cols=TARGET_ENC_COLS, target_col=TARGET,
    )

    # Spatial encoding — KD-tree from this fold's training set only
...
pd.DataFrame({"Id": test_raw[ID_COL], "Predicted": pred_a}).to_csv(
    '../submission/submission_v13_8020.csv', index=False
)
pd.DataFrame({"Id": test_raw[ID_COL], "Predicted": pred_b}).to_csv(
    '../submission/submission_v13_5fold.csv', index=False
)

print("\nSubmissions saved:")
print("  submission_v13_8020.csv   (80/20 split)")
print("  submission_v13_5fold.csv  (5-fold OOF  ← recommended)")

# ══════════════════════════════════════════════════════════════════════════════
# FEATURE IMPORTANCE  (Training A models — 80/20 split)
# Uses 'gain': total loss reduction from all splits on each feature.
# Training A is used because it produces a single model per algorithm,
# making importance straightforward to extract and compare.
# ══════════════════════════════════════════════════════════════════════════════

print("\n" + "═"*65)
print("Feature Importance — Training A (gain, normalised to %)")
print("═"*65)

feature_names = list(X_tr_a.columns)

# LightGBM — booster gain
imp_lgb = m_lgb_a.booster_.feature_importance(importance_type='gain')
imp_lgb = 100 * imp_lgb / imp_lgb.sum()

# XGBoost — get_score returns only features that were actually split on;
# unsplit features get 0
xgb_scores = m_xgb_a.get_booster().get_score(importance_type='gain')
imp_xgb = np.array([xgb_scores.get(f, 0.0) for f in feature_names])
imp_xgb = 100 * imp_xgb / imp_xgb.sum() if imp_xgb.sum() > 0 else imp_xgb

# CatBoost — PredictionValuesChange (comparable to gain for GBTs)
imp_cat = m_cat_a.get_feature_importance()
...
print("\nTop 30 features by mean gain across LGB / XGB / CAT:")
...
# Low-importance features — candidates to drop in a future version
