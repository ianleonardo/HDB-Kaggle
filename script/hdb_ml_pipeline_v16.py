"""
HDB Resale Price Prediction Pipeline — v16
...
Output   : submission_v16_5fold.csv

Changes vs v15
──────────────
1. Time-aware spatial encoding added (3 new features per fold):
     spatial_ta_500m_te   — smoothed mean price of training neighbours within
                            500 m AND within the last 24 months of the query
                            transaction. Captures recent local market conditions.
     spatial_ta_2000m_te  — same at 2 000 m (district-level recent trend).
     spatial_ta_500m_psm  — smoothed mean price-per-sqm within 500 m, last 24 months.

   Static spatial features (all-time neighbours) are KEPT alongside the
   time-aware ones — they capture the long-run neighbourhood price level,
   a complementary signal.

2. Why time-aware instead of replacing static?
     Static spatial  ≈ "what is this neighbourhood worth on average?"
     Time-aware      ≈ "what is this neighbourhood doing right now?"
   Both are useful; tranc_period (#10 feature in v14/v15) is the model's
   current proxy for the time signal — time-aware spatial offloads that
   work into the feature directly.

3. Time filter implementation: for each query point at period P, spatial
   neighbours are first found via cKDTree radius search, then filtered to
   training rows with tranc_period ∈ [P − 24, P]. If no filtered neighbours
   exist the row falls back to all spatial neighbours (prevents cold-start
   NaNs at the earliest periods in the dataset).

4. Hyperparameters frozen to v15 best (3-fold inner CV, 50 trials each).
   Total features per fold: 65 (was 62 in v15).
"""
...
LON_M          = 110_970.0
RADII_M        = [500, 2000]
TA_WINDOW_MONTHS = 24      # time window for time-aware spatial encoding
...
def spatial_radius_encode_fold(df_tr, df_val, df_te, target_col, radii_m=None, k=10):
    """Static (all-time) spatial encoding — KD-tree from df_tr only."""
    if radii_m is None:
        radii_m = RADII_M
    global_mean = df_tr[target_col].mean()
    ...
    return tr_enc, val_enc, te_enc, tree, prices_tr, psm_tr, global_mean, global_psm


def time_aware_spatial_encode_fold(df_tr, df_val, df_te, target_col,
                                   tree, prices_tr, psm_tr, global_mean, global_psm,
                                   radii_m=None, window=TA_WINDOW_MONTHS, k=10):
    """
    Time-aware spatial encoding — same KD-tree as static, but neighbours are
    filtered to those with tranc_period ∈ [query_period − window, query_period].

    Reuses the tree already built in spatial_radius_encode_fold to avoid
    rebuilding it.  Falls back to all spatial neighbours if the time filter
    leaves zero neighbours (prevents cold-start NaN at earliest periods).
    """
    if radii_m is None:
        radii_m = RADII_M

    periods_tr  = df_tr['tranc_period'].to_numpy()
    coords_tr   = _latlon_to_m(df_tr)
    coords_val  = _latlon_to_m(df_val)
    coords_te   = _latlon_to_m(df_te)

    def _encode_ta(query_coords, query_periods, values_tr, global_val, radius):
        all_nbrs = tree.query_ball_point(query_coords, r=radius, workers=-1)
        out = np.empty(len(query_coords))
        for i, (nbrs_i, qp) in enumerate(zip(all_nbrs, query_periods)):
            if len(nbrs_i) == 0:
                out[i] = global_val
            else:
                nbrs_arr = np.array(nbrs_i)
                # Keep only neighbours within the time window
                mask     = (periods_tr[nbrs_arr] >= qp - window) & \
                           (periods_tr[nbrs_arr] <= qp)
                ta_nbrs  = nbrs_arr[mask]
                if len(ta_nbrs) == 0:
                    # Cold-start fallback: use all spatial neighbours
                    out[i] = _smooth(len(nbrs_arr), values_tr[nbrs_arr].mean(), global_val, k)
                else:
                    out[i] = _smooth(len(ta_nbrs), values_tr[ta_nbrs].mean(), global_val, k)
        return out

    tr_periods  = df_tr['tranc_period'].to_numpy()
    val_periods = df_val['tranc_period'].to_numpy()
    te_periods  = df_te['tranc_period'].to_numpy()

    tr_enc, val_enc, te_enc = {}, {}, {}
    for r in radii_m:
        feat = f'spatial_ta_{r}m_te'
        tr_enc[feat]  = _encode_ta(coords_tr,  tr_periods,  prices_tr, global_mean, r)
        val_enc[feat] = _encode_ta(coords_val, val_periods, prices_tr, global_mean, r)
        te_enc[feat]  = _encode_ta(coords_te,  te_periods,  prices_tr, global_mean, r)
    r0 = min(radii_m)
    feat = f'spatial_ta_{r0}m_psm'
    tr_enc[feat]  = _encode_ta(coords_tr,  tr_periods,  psm_tr, global_psm, r0)
    val_enc[feat] = _encode_ta(coords_val, val_periods, psm_tr, global_psm, r0)
    te_enc[feat]  = _encode_ta(coords_te,  te_periods,  psm_tr, global_psm, r0)
    return tr_enc, val_enc, te_enc
...
DROP_COLS = [
    ...
    *REDUNDANT_COLS, *LOW_IMP_COLS, *TARGET_ENC_COLS, *LABEL_ENC_COLS,
]

BASE_FEATURES = [c for c in train_fe.columns if c not in DROP_COLS]

STATIC_SPATIAL_FEATS = [f'spatial_{r}m_te' for r in RADII_M] + \
                       [f'spatial_{min(RADII_M)}m_psm']
TA_SPATIAL_FEATS     = [f'spatial_ta_{r}m_te' for r in RADII_M] + \
                       [f'spatial_ta_{min(RADII_M)}m_psm']
ALL_ENC_FEATS        = TARGET_ENC_COLS + STATIC_SPATIAL_FEATS + TA_SPATIAL_FEATS
n_total = len(BASE_FEATURES) + len(TARGET_ENC_COLS) + len(STATIC_SPATIAL_FEATS) + len(TA_SPATIAL_FEATS)

print(f"\nBase features                : {len(BASE_FEATURES)}")
print(f"Target-encoded (per fold)   : {TARGET_ENC_COLS}")
print(f"Static spatial (per fold)   : {STATIC_SPATIAL_FEATS}")
print(f"Time-aware spatial (per fold): {TA_SPATIAL_FEATS}  [window={TA_WINDOW_MONTHS} months]")
...
print("\n" + "═"*65)
print(f"Training — {N_FOLDS}-fold OOF  (static + time-aware spatial, leak-free)")
print("═"*65)
...
for fold, (tr_idx, val_idx) in enumerate(kf.split(np.arange(len(train_fe))), 1):
    ...
    # Static spatial encoding — also returns tree + arrays for reuse
    print(f"    Building KD-tree + static spatial encodings...", end=' ', flush=True)
    sp_tr, sp_val, sp_te, tree, prices_tr, psm_tr, g_mean, g_psm = \
        spatial_radius_encode_fold(df_tr=df_tr_f, df_val=df_val_f, df_te=test_fe,
                                   target_col=TARGET)
    print("done")

    # Time-aware spatial encoding — reuses the same tree
    print(f"    Time-aware spatial encodings (window={TA_WINDOW_MONTHS}m)...", end=' ', flush=True)
    ta_tr, ta_val, ta_te = time_aware_spatial_encode_fold(
        df_tr=df_tr_f, df_val=df_val_f, df_te=test_fe, target_col=TARGET,
        tree=tree, prices_tr=prices_tr, psm_tr=psm_tr,
        global_mean=g_mean, global_psm=g_psm,
    )
    print("done")

    tr_enc  = {**tr_enc,  **sp_tr,  **ta_tr}
    val_enc = {**val_enc, **sp_val, **ta_val}
    te_enc  = {**te_enc,  **sp_te,  **ta_te}
...
v16_rmse = root_mean_squared_error(y, oof_blend)
delta    = V15_OOF_RMSE - v16_rmse
print(f"\n  v15 ensemble OOF RMSE : {V15_OOF_RMSE:,}")
print(f"  v16 ensemble OOF RMSE : {v16_rmse:,.0f}  "
...
pd.DataFrame({"Id": test_raw[ID_COL], "Predicted": pred_final}).to_csv(
    '../submission/submission_v16_5fold.csv', index=False
)
print("\nSubmission saved: submission_v16_5fold.csv")
...
# Tag time-aware features for easy identification
ta_feats = set(TA_SPATIAL_FEATS)
df_imp['tag'] = df_imp['feature'].apply(lambda f: ' [TA]' if f in ta_feats else '')

print(f"\n  {'Feature':<45} {'LGB%':>6}  {'XGB%':>6}  {'CAT%':>6}  {'Mean%':>6}")
print("  " + "-"*71)
for _, row in df_imp.head(35).iterrows():
    print(f"  {row['feature'] + row['tag']:<45} {row['lgb_%']:>6.2f}  {row['xgb_%']:>6.2f}"
...
print(f"\nTime-aware spatial features summary:")
for feat in TA_SPATIAL_FEATS:
    row = df_imp[df_imp['feature'] == feat].iloc[0]
    rank = df_imp[df_imp['feature'] == feat].index[0] + 1
    print(f"  {feat:<35}  rank={rank:>2}  mean={row['mean_%']:.2f}%")
