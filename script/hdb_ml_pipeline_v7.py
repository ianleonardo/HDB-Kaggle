"""
HDB Resale Price Prediction Pipeline  —  v7
Primary model : CatBoost (Optuna-tuned)
Secondary     : LightGBM, XGBoost
Metric        : RMSE
Validation    : 5-fold KFold OOF stacking

Changes vs v6:
  - Optuna HPO for CatBoost (50 trials, single 80/20 inner split for speed)
    Search space: depth, l2_leaf_reg, random_strength, bagging_temperature,
                  border_count, learning_rate
    Best params saved and used for the full 5-fold OOF phase.
  - GPU support via USE_GPU flag (set False if no NVIDIA GPU)
    CatBoost: task_type='GPU'  — 5-10x speedup, makes HPO practical
    XGBoost : device='cuda'    — 3-5x speedup
    LightGBM: CPU only         — GPU build not worth it on 150k rows
"""
...
XGB_DEVICE    = 'cuda' if USE_GPU else None  # None = XGBoost default (CPU)

print(f"GPU mode: {USE_GPU}  (CatBoost={CAT_TASK_TYPE}, XGBoost device={XGB_DEVICE})")
...
# ─── Optuna HPO — CatBoost ────────────────────────────────────────────────────
# Single 80/20 inner split for speed. Best params then used in full 5-fold OOF.
print("\n" + "="*65)
print("Phase 1: Optuna HPO for CatBoost (50 trials)")
print("="*65)

X_hpo_tr, X_hpo_val, y_hpo_tr, y_hpo_val = train_test_split(
    X, y, test_size=0.20, random_state=0   # different seed from OOF to avoid bias
)
...
print(f"\nBest CatBoost params (trial #{study.best_trial.number}, "
      f"HPO val RMSE={study.best_value:,.0f}):")
...
# ─── 5-Fold OOF Training ──────────────────────────────────────────────────────
print("\n" + "="*65)
print("Phase 2: 5-fold OOF training with best params")
print("="*65)
...
for fold, (tr_idx, val_idx) in enumerate(kf.split(X), 1):
    ...
    # CatBoost (GPU if available, Optuna-tuned params)
...
# ─── Summary ─────────────────────────────────────────────────────────────────
print(f"\n{'='*65}")
print(f"=== OOF Metrics (5-fold, full training set) ===")
...
sub.to_csv('../submission/submission_ensemble_v7.csv', index=False)
print("\nSubmission saved to submission/submission_ensemble_v7.csv")
print(sub.head())
