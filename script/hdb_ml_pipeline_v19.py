"""
HDB Resale Price Prediction Pipeline — v19
...
Output   : submission_v19_5fold.csv

Changes vs v15
──────────────
1. Added spatial_2000m_psm — smoothed mean price-per-sqm within 2000 m.
   Previously psm was only computed at the finest radius (500 m). The 2000 m
   psm captures district-level size-normalised price, a complementary signal
   to the 500 m psm which is block-level.

   Spatial features per fold (4 total, was 3):
     spatial_500m_te    — mean price within 500 m   (block-level)
     spatial_2000m_te   — mean price within 2000 m  (district-level)
     spatial_500m_psm   — mean price/sqm within 500 m
     spatial_2000m_psm  — mean price/sqm within 2000 m  ← new

2. No other changes — isolated experiment vs v15.
   Total features per fold: 63 (was 62 in v15).
"""
...
PROXY_SPATIAL_FEATS = [f'spatial_{r}m_te_proxy' for r in RADII_M] + \
                      [f'spatial_{r}m_psm_proxy' for r in RADII_M]
...
v19_rmse = root_mean_squared_error(y, oof_blend)
delta    = V15_OOF_RMSE - v19_rmse
print(f"\n  v15 ensemble OOF RMSE : {V15_OOF_RMSE:,}")
print(f"  v19 ensemble OOF RMSE : {v19_rmse:,.0f}  "
...
pd.DataFrame({"Id": test_raw[ID_COL], "Predicted": pred_final}).to_csv(
    '../submission/submission_v19_5fold.csv', index=False
)
print("\nSubmission saved: submission_v19_5fold.csv")
