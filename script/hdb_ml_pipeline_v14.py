...
# Low-importance raw columns identified from v13 feature importance (mean gain < 0.10%)
# plus raw coordinates now absorbed by spatial encoding.
# Only original CSV columns appear here — engineered columns that were dropped
# have simply been removed from feature_engineering() instead.
LOW_IMP_COLS = [
    # Rental unit counts
    ...
    'multistorey_carpark', 'precinct_pavilion',
    # Interchange flags (raw string; binary version removed from feature_engineering)
    ...
    # Raw coordinates (absorbed by spatial encoding; still used internally by
    # spatial_radius_encode_fold via _latlon_to_m — just not passed to models)
    'Latitude', 'Longitude',
]
...
def feature_engineering(df, mall_dist_median):
    ...
    # Distances (log-transformed; raw sec/pri/bus distances kept as-is from CSV)
    ...
    # School quality composites (use raw affiliation cols as inputs, then drop raws via LOW_IMP_COLS)
