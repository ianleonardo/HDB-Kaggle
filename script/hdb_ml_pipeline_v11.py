"""
HDB Resale Price Prediction Pipeline — v11
...
Outputs  : Two submissions
             submission_v11_8020.csv  — 80/20 random split
             submission_v11_5fold.csv — 5-fold OOF (recommended)

Key change vs final.py — leakage-free target encoding:
  Previous: town/planning_area encoded on full training set BEFORE splitting.
            Validation rows' own prices contributed to their encoding → leakage.
  v11:      Encoding computed INSIDE each training phase using training rows only.
            Validation and test rows are mapped using training statistics.
            Each OOF fold gets its own independent encoding — zero leakage.
"""
...
def target_encode_fold(df_tr, df_val, df_te, cols, target_col, k=10):
    ...
    Small groups (few transactions) shrink toward the global mean.
...
# ══════════════════════════════════════════════════════════════════════════════
# FEATURE SELECTION
# town / planning_area excluded from BASE_FEATURES here — they are added
# as _te columns inside each training phase after leakage-free encoding.
# ══════════════════════════════════════════════════════════════════════════════
...
print(f"\nBase features (before target encoding) : {len(BASE_FEATURES)}")
print(f"Target-encoded columns added per fold  : {TARGET_ENC_COLS}")
print(f"Total features per fold                : {len(BASE_FEATURES) + len(TARGET_ENC_COLS)}")
...
# Compute encoding from training rows only
tr_enc_a, val_enc_a, te_enc_a = target_encode_fold(
    df_tr  = train_fe.iloc[tr_idx_a],   # stats computed here
    df_val = train_fe.iloc[val_idx_a],  # mapped using tr stats
    df_te  = test_fe,                   # mapped using tr stats
    cols   = TARGET_ENC_COLS,
    target_col = TARGET,
)

X_tr_a   = attach_encodings(train_fe.iloc[tr_idx_a][BASE_FEATURES], tr_enc_a)
X_val_a  = attach_encodings(train_fe.iloc[val_idx_a][BASE_FEATURES], val_enc_a)
...
for fold, (tr_idx, val_idx) in enumerate(kf.split(np.arange(len(train_fe))), 1):
    ...
    tr_enc, val_enc, te_enc = target_encode_fold(
        df_tr  = train_fe.iloc[tr_idx],
        df_val = train_fe.iloc[val_idx],
        df_te  = test_fe,
        cols   = TARGET_ENC_COLS,
        target_col = TARGET,
    )

    X_tr  = attach_encodings(train_fe.iloc[tr_idx][BASE_FEATURES],  tr_enc)
    X_val = attach_encodings(train_fe.iloc[val_idx][BASE_FEATURES], val_enc)
...
pd.DataFrame({"Id": test_raw[ID_COL], "Predicted": pred_a}).to_csv(
    '../submission/submission_v11_8020.csv', index=False
)
pd.DataFrame({"Id": test_raw[ID_COL], "Predicted": pred_b}).to_csv(
    '../submission/submission_v11_5fold.csv', index=False
)

print("\nSubmissions saved:")
print("  submission_v11_8020.csv   (80/20 split)")
print("  submission_v11_5fold.csv  (5-fold OOF  ← recommended)")
